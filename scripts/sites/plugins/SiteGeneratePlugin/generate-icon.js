...
const { renderNavIntro, renderReadmeSource } = require('./generate-site');
...
function generateIconDemoByLang(language) {
    ...
    try {
        ...
        const { source: introSource } = renderNavIntro(readmeSplit[0]);
        const { source: propsSource } = renderReadmeSource(readmeSplit[1]);
