    ...
    return (
        <div className='typical-demo-card typical-demo-list1-box'>
            <div className='tag-group'>
                {
                    [1,1,1].map((_, index) => <Tag key={index} size="large" borderStyle="none" filleted {...(activeIndex === index ? {} : defaultStyle)} onClick={() => setActiveIndex(index)}>{localeMap.ListExample[language]}</Tag>)
                }
            </div>
