...
describe('Popover', () => {
    ...
    it('should open and close popover correctly', () => {
        ...
        const map = mockAddListener(document.body, true);
        ...
        wrapper.setProps({ clickSelfToClose: true, clickOtherToClose: true, preventBodyClick: true });
        clickButton(wrapper);
    ...
    it('should adjust itself correctly when shown on the edge', () => {
        ...
        child.getBoundingClientRect = jest.fn(() => ({ top: 600, bottom: 690, left: 10, right: 100, width: 90, height: 90 }));
        updatePopoverPosition(wrapper, ref);
        expect(wrapper.find('.popover-content').hasClass('topCenter')).toBe(true);
        // top overflow
        mockPopoverSize(ref, 375, 100);
        child.getBoundingClientRect = jest.fn(() => ({ top: 10, bottom: 100, left: 10, right: 100, width: 90, height: 90 }));
    ...
    it('should support Popover.Menu', () => {
        ...
        const wrapper = mount(<Popover.Menu
            onChange={onChange}
            onSelect={onSelect}
            menu={[
                {
        ...
        expect(wrapper.find(`.${prefix}-menu-item`).at(2).find(`.${prefix}-menu-icon`)).toHaveLength(1);
        ...
        expect(onSelect.mock.calls[0]).toEqual(['first', {
            value: 'first',
            text: 'Menu 1',
        }]);
        expect(onChange.mock.calls).toHaveLength(1);
        wrapper.setProps({ useClickStatus: true, clickStatusDuration: 500, clickSelfToClose: true });
